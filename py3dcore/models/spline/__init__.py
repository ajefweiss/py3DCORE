...
class SplineModel(SimulationBlackBox):
    ...
    def __init__(
        ...
        particles: int = 11,
        iparams: dict = {},
        shape_model: str = "torus",
        mag_model: str = "gh",
        dtype: type = np.float32,
    ) -> None:
        with open(
            os.path.join(
                os.path.dirname(py3dcore.__file__), "models/spline/parameters.json"
            )
        ) as fh:
    ...
    def propagator(
        self, dt_to: Union[str, datetime.datetime], time_resolution: int = 5 * 3600
    ) -> None:
        ...
        _numba_generate_csplines(
            self.sparams_arr, lmatrix, self.particles, self.cscoeff, self.cscoeff_v
        )
    ...
    def visualize_fieldlines(
        self, q0: np.ndarray, iparam_index: int = 0, step_size: float = 0.001
    ) -> np.ndarray:
        _q = np.empty((12,))
        _s = np.empty((3,))
        _b = np.empty((3,))
        ...
        csplines_qs(
            ...
            self.cscoeff[iparam_index],
            self.cscoeff_v[iparam_index],
            _s,
            _s,
        )
            ...
            try:
                sol = getattr(
                    least_squares(
                        lambda x: x
                        - fll[-1]
                        - step_size * iter((x.astype(self.dtype) + fll[-1]) / 2),
                        fll[-1],
                        method="trf",
                    ),
                    "x",
...
def _numba_generate_csplines(
    sparams_arr: np.ndarray,
    Lmat: np.ndarray,
    pn: int,
    cscoeff_x: np.ndarray,
    cscoeff_v: np.ndarray,
) -> None:
    h = 1 / (pn - 1)
...
def _numba_propagator(
    ...
    for i in numba.prange(len(iparams_arr)):
        ...
        for k in range(pn):
            ...
            v_sw_eff = _numba_get_sw_vr(
                t_offset, sparams_arr[i, k, :3], sw_data, sw_rmax, sw_rmin
            )
        ...
        sparams_arr[i, :, 0] += (
            sparams_arr[i, :, 3] * (1 + a_fac) / 2 / 1.496e8 * t_offset
        )
        sparams_arr[i, :, 1] += (
            sparams_arr[i, :, 4] * (1 + a_fac) / 2 / 1.496e8 * t_offset
        )
        sparams_arr[i, :, 2] += (
            sparams_arr[i, :, 5] * (1 + a_fac) / 2 / 1.496e8 * t_offset
        )
...
def _numba_pfunc_gcslike(r: float, p: float) -> np.ndarray:
    return np.array(
        [0, -r / 2 * np.cos(p) + r / 2, r / 2 * np.sin(p) * np.abs(np.sin(p / 2)), 0]
    )
...
def _numba_pfunc_gcslike_curved(r: float, p: float) -> np.ndarray:
    ...
    return np.array(
        [
            0,
            -r / 2 * np.cos(p) + r / 2,
            r / 2 * np.sin(p) * np.abs(np.sin(p / 2)),
            r / 25 * np.sin(pred),
        ]
    )
