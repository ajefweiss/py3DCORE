...
from py3dcore.models.thin_torus_nc.coordinates import g, f
from py3dcore.models.thin_torus_nc.magfield import h
from py3dcore.models.thin_torus_nc.propagation import p


class ThinTorusNC3DCOREModel(Toroidal3DCOREModel):
    """Implements the TorusNC3DCOREModel model.
    ...
    def __init__(self, launch, runs, use_gpu=False, **kwargs):
        ...
        parameters = kwargs.pop("parameters", thin_torus_nc_parameters)

        if isinstance(parameters, dict):
            parameters = Base3DCOREParameters(parameters, dtype=dtype)

        super(ThinTorusNC3DCOREModel, self).__init__(
            launch, funcs, parameters, sparams_count=5, runs=runs, use_gpu=use_gpu, **kwargs
        )

    @classmethod
    def default_parameters(cls):
        return dict(thin_torus_nc_parameters)


thin_torus_nc_parameters = {
    "cme_launch_offset": {
        "index": 0,
        "name": "launch time",
        "distribution": "fixed",
